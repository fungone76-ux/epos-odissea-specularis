...
Python owns calendar, event eligibility, mission gates, scoring, rewards and
information disclosure. The LLM interprets only the state already validated by
this module and cannot complete missions through narration alone.
"""
...
from .models import RELATIONSHIP_DOMAINS, WorldState
from .resort_relationships import (
    RESORT_NPC_IDS,
    initialise_resort_relationships,
    relationship_snapshot,
)
from .worldpack import MissionDef, WorldPack, WorldPackError, load_pack
...
def load_resort_pack(pack_dir: str | Path) -> ResortPack:
    ...
    for index, raw in enumerate(mission_data.get("missions", [])):
        ...
        success_points = int(raw.get("success_points", 0))
        failure_points = int(raw.get("failure_points", 0))
        if success_points < 0:
            raise WorldPackError(f"missione {mission_id}: success_points non può essere negativo")
        if failure_points > 0:
            raise WorldPackError(f"missione {mission_id}: failure_points deve essere zero o negativo")
    ...
    for index, raw in enumerate(event_data.get("events", [])):
        ...
        if bool(raw.get("consent_exit_required", False)) and not any(
            ...
            for token in ("decline", "cancel", "skip", "leave", "alone", "postpone", "end", "refuse")
        ):
...
def initialise_resort_state(state: WorldState) -> WorldState:
    """Initialise persistent campaign fields without changing core models."""
    ...
    state.flags.setdefault("resort_completed_missions", [])
    state.flags.setdefault("resort_failed_missions", [])
    state.flags.setdefault("resort_mission_status", {"mission_resort_future": "active"})
    state.flags.setdefault("resort_rewarded_missions", [])
    state.flags.setdefault("resort_campaign_score", 0)
    state.flags.setdefault("resort_score_history", [])
...
def new_resort_world(pack: ResortPack, session_id: str | None = None) -> WorldState:
    return initialise_resort_state(pack.world.new_world(session_id))
...
def campaign_score(state: WorldState) -> int:
    initialise_resort_state(state)
    return int(state.flags.get("resort_campaign_score", 0))


def campaign_score_tier(state: WorldState) -> str:
    score = campaign_score(state)
    if score >= 65:
        return "finale_perfetto"
    if score >= 50:
        return "successo_eccellente"
    if score >= 35:
        return "settimana_positiva"
    if score >= 20:
        return "risultato_incompleto"
    return "soggiorno_fallimentare"
...
def _trigger_matches(state: WorldState, trigger: dict[str, Any]) -> bool:
    ...
    if "relationship_field" in trigger:
        ...
        if field == "jealousy":
            return any(
                int(state.flags.get(f"resort_jealousy_{npc_id}", 0)) >= minimum
...
def _relationship_field(field: str) -> str:
    aliases = {"professional_respect": "respect"}
    normalized = aliases.get(field, field)
    if normalized not in RELATIONSHIP_DOMAINS:
        raise WorldPackError(f"ricompensa relazione con campo sconosciuto: {field!r}")
    return normalized


def _apply_mission_rewards(state: WorldState, mission: MissionDef) -> list[dict[str, Any]]:
    applied: list[dict[str, Any]] = []
    for reward in mission.rewards:
        reward_type = str(reward.get("type", ""))
        if reward_type == "flag":
            key = str(reward.get("key", "")).strip()
            if not key:
                raise WorldPackError(f"missione {mission.id}: ricompensa flag senza key")
            value = reward.get("value", True)
            state.flags[key] = value
            applied.append({"type": "flag", "key": key, "value": value})
        elif reward_type == "relationship":
            target_id = str(reward.get("target_id", ""))
            if target_id not in state.npcs:
                raise WorldPackError(
                    f"missione {mission.id}: ricompensa verso NPC sconosciuto {target_id!r}"
                )
            field = _relationship_field(str(reward.get("field", "")))
            delta = int(reward.get("delta", 0))
            relationship = state.npcs[target_id].relationship_towards("player")
            relationship.apply_delta({field: delta})
            applied.append(
                {"type": "relationship", "target_id": target_id, "field": field, "delta": delta}
            )
        else:
            raise WorldPackError(
                f"missione {mission.id}: tipo ricompensa non supportato {reward_type!r}"
            )
    return applied


def _all_flags_true(state: WorldState, flags: tuple[str, ...]) -> bool:
    return bool(flags) and all(bool(state.flags.get(flag, False)) for flag in flags)


def evaluate_resort_missions(state: WorldState, pack: ResortPack) -> list[dict[str, Any]]:
    """Resolve active missions once, apply rewards and persist campaign score.

    Success and failure are driven exclusively by authoritative Python flags.
    Rewards and score changes are idempotent: repeated evaluation cannot grant
    the same mission twice.
    """

    initialise_resort_state(state)
    active = set(state.flags.get("resort_active_missions", []))
    completed = set(state.flags.get("resort_completed_missions", []))
    failed = set(state.flags.get("resort_failed_missions", []))
    rewarded = set(state.flags.get("resort_rewarded_missions", []))
    statuses = dict(state.flags.get("resort_mission_status", {}))
    history = list(state.flags.get("resort_score_history", []))
    changes: list[dict[str, Any]] = []

    for mission_id in sorted(active):
        if mission_id in completed or mission_id in failed:
            continue
        policy = pack.mission_policies.get(mission_id)
        mission = pack.world.missions.get(mission_id)
        if policy is None or mission is None:
            raise WorldPackError(f"missione attiva senza definizione completa: {mission_id}")

        failed_now = _all_flags_true(state, policy.terminal_failure_flags)
        succeeded_now = _all_flags_true(state, policy.terminal_success_flags)
        if not failed_now and not succeeded_now:
            statuses[mission_id] = "active"
            continue

        if failed_now:
            failed.add(mission_id)
            statuses[mission_id] = "failed"
            delta = policy.failure_points
            state.flags["resort_campaign_score"] = campaign_score(state) + delta
            entry = {
                "mission_id": mission_id,
                "status": "failed",
                "score_delta": delta,
                "score_total": campaign_score(state),
                "rewards": [],
            }
        else:
            completed.add(mission_id)
            statuses[mission_id] = "completed"
            rewards: list[dict[str, Any]] = []
            if mission_id not in rewarded:
                rewards = _apply_mission_rewards(state, mission)
                rewarded.add(mission_id)
            delta = policy.success_points
            state.flags["resort_campaign_score"] = campaign_score(state) + delta
            entry = {
                "mission_id": mission_id,
                "status": "completed",
    ...
    state.flags["resort_completed_missions"] = sorted(completed)
    state.flags["resort_failed_missions"] = sorted(failed)
    state.flags["resort_rewarded_missions"] = sorted(rewarded)
    state.flags["resort_mission_status"] = statuses
    state.flags["resort_score_history"] = history
    state.flags["resort_score_tier"] = campaign_score_tier(state)
    return changes
...
def update_mission_unlocks(state: WorldState, pack: ResortPack) -> list[str]:
    ...
    for policy in pack.mission_policies.values():
        ...
        if "flag" in rule:
            ...
            unlocked = str(rule["event_completed"]) in state.flags.get(
                "resort_completed_events", []
            )
        elif "relationship" in rule:
            npc_id = str(rule["relationship"])
            field = _relationship_field(str(rule.get("field", "trust")))
            minimum = int(rule.get("minimum", 0))
            if npc_id in state.npcs:
                unlocked = (
                    int(getattr(state.npcs[npc_id].relationship_towards("player"), field))
                    >= minimum
                )
        if unlocked:
            active.add(policy.id)
            revealed.add(policy.id)
            statuses[policy.id] = "active"
...
def update_luna_disclosure_gates(state: WorldState) -> None:
    ...
    if bool(state.flags.get("luna_letter_revealed", False)) and bool(
        state.flags.get("luna_letter_author_confirmed", False)
    ):
        state.flags["luna_letter_truth_unlocked"] = True


def process_resort_turn(
    state: WorldState,
    pack: ResortPack,
    _result: Any | None = None,
) -> dict[str, Any]:
    """Canonical post-turn processor for the Resort campaign."""

    update_luna_disclosure_gates(state)
    unlocked = update_mission_unlocks(state, pack)
    mission_changes = evaluate_resort_missions(state, pack)
    return {
...
def llm_context_for_npc(state: WorldState, pack: ResortPack, npc_id: str) -> dict[str, Any]:
    ...
    active_missions = [
        ...
        if npc_id in mission_id
        or (npc_id == "victoria" and mission_id == "mission_victoria_save_resort")
    ]
    return {
        ...
        "identity": {
            "name": canon.name,
            "age": canon.age,
            "personality": list(canon.personality),
        },
        ...
        "outfit": {
            "worn": list(npc.outfit.worn),
            "removed": list(npc.outfit.removed),
        },
        "disclosure_policy": canon.disclosure_policy,
        "red_lines": list(canon.red_lines),
        "intimate_profile": canon.intimate_profile,
        "rules": [
            "Non inventare avanzamenti, flag, punteggi, cambi di luogo o outfit.",
