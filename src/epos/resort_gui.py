...
Riutilizza integralmente EposGameWindow per input libero, worker LLM, prove,
chat, rendering e salvataggi. Aggiunge soltanto il pannello e i messaggi di
campagna specifici del Resort, senza dipendenze dall'Odissea.
"""
...
def build_resort_gui_status(state, pack: ResortPack) -> dict[str, Any]:
    ...
    La funzione non modifica il mondo ed è testabile senza avviare Qt.
    """
    ...
    days_left = max(0, int(state.flags.get("resort_debt_due_day", 7)) - current_day(state))
    ...
    return {
        ...
        "debt_due": debt_due,
        "days_left": days_left,
        "completed_missions": list(state.flags.get("resort_completed_missions", [])),
...
class ResortGameWindow(EposGameWindow):
    ...
    def _refresh_state_panel(self) -> None:
        ...
        self._add_state_widget(
            layout,
            self._label(f"Presenti: {npcs}", "color: #9ec5fe; font-size: 10px;"),
        )
        ...
        if status["missions"]:
            ...
            for mission in status["missions"]:
                ...
                style = "color: #9bd59b; font-size: 9px;" if mission["status"] == "completed" else "color: #c8ced8; font-size: 9px;"
    ...
    def _on_result(self, payload) -> None:
        ...
        if kind != "turn" or not isinstance(data, TurnResult):
            return
        ...
        for change in changes.get("mission_changes", []):
            ...
            status = "COMPLETATA" if change.get("status") == "completed" else "FALLITA"
            self._append_system(
                f"MISSIONE {status}: {name} · {sign}{delta} punti · totale {change.get('score_total', campaign_score(self.state))}"
            )
