...
class ResortGameWindow(EposGameWindow):
    ...
    def __init__(
        ...
        self._install_manual_time_button()
    ...
    def _install_manual_time_button(self) -> None:
        ...
        button.clicked.connect(self._advance_time_clicked)
        layout.insertWidget(max(layout.count() - 1, 0), button)
        self._manual_time_button = button

    def _advance_time_clicked(self) -> None:
        callback = getattr(self, "manual_time_callback", None)
        if not callable(callback):
            self._append_system("Avanzamento del tempo non configurato.")
            return

        button = getattr(self, "_manual_time_button", None)
        ...
        try:
            ...
            if result.get("advanced"):
                ...
                self._append_system(f"TEMPO AVANZATO: giorno {day} - {phase}")
                ...
                self._append_system(
                    str(result.get("message", "Il tempo non e stato modificato."))
                )
            self._refresh_state_panel()
            self._scroll_to_bottom()
        except Exception as exc:
            self._append_system(f"Errore durante l'avanzamento del tempo: {exc}")
        finally:
            if button is not None:
